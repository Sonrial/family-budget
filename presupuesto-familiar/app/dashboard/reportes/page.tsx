    ...
    async function loadReport() {
      try {
        ...
        let query = client.from('transactions')
          .select('*, amount:transaction_lines(amount, account:accounts(id,name,type))')
        ...
        const { data, error } = await query.order('id').range(offset, offset + 499).returns<ReportTransaction[]>()
        if (error) throw error

        transactions.push(...(data ?? []))
        if ((data?.length ?? 0) < 500 || cancelled) break
        }
        ...
        if (!cancelled) { setPieData([]); setTopExpenses([]); setKpis(emptyReportKpis); toast.error(getFinanceErrorMessage(error)); setLoading(false) }
      }
    }
    void loadReport()
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Reportes" description="Ingresos, consumo, ahorro y pagos de deuda correctamente separados."
        actions={<><ScopeToggle value={scope} onChange={changeScope} /><Select value={selectedMonth} onValueChange={(value) => { if (value !== selectedMonth) { setLoading(true); setSelectedMonth(value) } }}><SelectTrigger className="w-[190px]" aria-label="Mes del reporte"><SelectValue /></SelectTrigger><SelectContent>{months.map((month) => <SelectItem key={month.value} value={month.value}>{month.label}</SelectItem>)}</SelectContent></Select></>} />
      ...
      {loading ? <LoadingState label="Generando reporte…" /> : (
        <>
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <MetricCard title="Ahorro neto" value={kpis.savings} helper={kpis.income ? `Tasa de ahorro ${kpis.savingsRate.toFixed(1)}%` : 'Tasa no aplicable sin ingresos'} icon={PiggyBank} tone={kpis.savings >= 0 ? 'blue' : 'red'} />
          </div>

          <div className="grid gap-5 xl:grid-cols-2">
            <Card><CardHeader><CardTitle>Distribución del gasto</CardTitle><CardDescription>Solo categorías de consumo.</CardDescription></CardHeader><CardContent className="h-80">{pieData.length ? <ResponsiveContainer width="100%" height="100%"><PieChart accessibilityLayer><Pie data={pieData} dataKey="value" nameKey="name" innerRadius={65} outerRadius={100} paddingAngle={3}>{pieData.map((datum, index) => <Cell key={datum.id} fill={colors[index % colors.length]} />)}</Pie><Tooltip content={<ChartTooltip />} /><Legend formatter={(value) => <span className="text-foreground">{value}</span>} /></PieChart></ResponsiveContainer> : <EmptyState title="Sin gastos" description="No se registraron gastos de consumo en este periodo." />}</CardContent></Card>
            <Card><CardHeader><CardTitle>Flujo del periodo</CardTitle><CardDescription>Comparación entre ingresos, consumo y deuda.</CardDescription></CardHeader><CardContent className="h-80"><ResponsiveContainer width="100%" height="100%"><BarChart accessibilityLayer data={[{ name: 'Ingresos', amount: kpis.income }, { name: 'Consumo', amount: kpis.expense }, { name: 'Deuda', amount: kpis.debtPayments }]}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)" vertical={false} /><XAxis tick={{ fill: 'var(--muted-foreground)' }} dataKey="name" tickLine={false} axisLine={false} /><YAxis hide /><Tooltip content={<ChartTooltip />} /><Bar dataKey="amount" radius={[8, 8, 0, 0]}>{['#059669', '#dc2626', '#f59e0b'].map((color) => <Cell key={color} fill={color} />)}</Bar></BarChart></ResponsiveContainer></CardContent></Card>
          </div>

          <div className="grid gap-5 xl:grid-cols-2">
            <Card><CardHeader><CardTitle>Detalle por categoría</CardTitle><CardDescription>Participación sobre el gasto de consumo.</CardDescription></CardHeader><CardContent>{pieData.length ? <div className="space-y-4">{pieData.map((category, index) => <div key={category.id}><div className="mb-1.5 flex justify-between gap-3 text-sm"><span className="font-medium">{category.name}</span><span className="metric-value font-semibold">{formatCurrency(category.value)}</span></div><div className="h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full" style={{ width: `${category.percent}%`, background: colors[index % colors.length] }} /></div><p className="mt-1 text-right text-xs text-muted-foreground">{category.percent.toFixed(1)}%</p></div>)}</div> : <EmptyState title="Sin categorías" description="No hay datos para distribuir." />}</CardContent></Card>
            <Card><CardHeader><div className="flex items-center gap-2"><AlertTriangle className="size-4 text-amber-600 dark:text-amber-300" /><CardTitle>Mayores gastos</CardTitle></div><CardDescription>Top cinco del periodo.</CardDescription></CardHeader><CardContent>{topExpenses.length ? <div className="divide-y">{topExpenses.map((expense, index) => <div key={`${expense.description}-${expense.date}-${index}`} className="flex items-center justify-between gap-4 py-3"><div className="min-w-0"><p className="truncate text-sm font-medium">{expense.description}</p><p className="text-xs text-muted-foreground">{expense.category} · {expense.date.slice(0, 10)}</p></div><p className="metric-value shrink-0 text-sm font-bold text-red-600 dark:text-red-300">{formatCurrency(expense.amount)}</p></div>)}</div> : <EmptyState title="Sin gastos" description="No hay compras para mostrar." />}</CardContent></Card>
          </div>
        </>
      )}
    </div>
...
function MetricCard({ title, value, helper, icon: Icon, tone }: { title: string; value: number; helper: string; icon: typeof TrendingUp; tone: 'blue' | 'emerald' | 'red' | 'amber' }) {
  const tones = { blue: 'bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-300', emerald: 'bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300', red: 'bg-red-50 dark:bg-red-950/50 text-red-700 dark:text-red-300', amber: 'bg-amber-50 dark:bg-amber-950/50 text-amber-700 dark:text-amber-300' }
