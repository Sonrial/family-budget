      ...
      {loading ? <LoadingState label="Generando reporte…" /> : (
        <>
          ...
          <div className="grid gap-5 xl:grid-cols-2">
            <Card><CardHeader><CardTitle>Distribución del gasto</CardTitle><CardDescription>Solo categorías de consumo.</CardDescription></CardHeader><CardContent className="h-80">{pieData.length ? <ResponsiveContainer width="100%" height="100%"><PieChart accessibilityLayer><Pie data={pieData} dataKey="value" nameKey="name" innerRadius={65} outerRadius={100} paddingAngle={3}>{pieData.map((datum, index) => <Cell key={datum.id} fill={colors[index % colors.length]} />)}</Pie><Tooltip content={<ChartTooltip />} /><Legend /></PieChart></ResponsiveContainer> : <EmptyState title="Sin gastos" description="No se registraron gastos de consumo en este periodo." />}</CardContent></Card>
