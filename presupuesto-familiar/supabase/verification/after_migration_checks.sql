...
-- 1. Ningún asiento debe quedar desbalanceado.
select
  t.id,
  t.date,
  t.description,
...
-- 2. Detectar cabeceras sin líneas o con una sola línea.
...
-- 3. Verificar que todas las filas compartidas tengan hogar.
...
-- 4. Totales y conteos de control para comparar con producción.
...
-- 5. Saldos calculados sin modificar registros.
select id, name, type, scope, current_balance
from public.account_balances_v2
order by scope, type, name;

-- 6. Cada reversión debe cancelar exactamente, cuenta por cuenta, al original.
select
...
-- 7. No debe haber reversiones sin original ni originales sin marca de anulación.
...
-- 8. Los marcadores de pago deben apuntar a movimientos vigentes.
